...
public class HW22 {
	 ...
	 * </ul>
	 * <li>3. convert the final stream into a list. Ex: ["cbe", "ec", "ddeef" ] --> ["cbe", "ddeef"]
	 * </ol>
	 */
	public static List<String> FindAllwordsPassingASeqOfTests(String... ws) {
		...
		// put your code here!
		// Written by Richard YC. Tsai
		Stream<String> strm = Stream.of(ws);
		
		return strm.filter(w -> w.length() < 8)
				   .filter(w -> w.contains(String.valueOf('e')))
	...
	public static String words2UpperCaseWord(List<String> ws) {
		...
		// Written by Richard YC. Tsai
		return ws.stream().map(s->s.toUpperCase()).reduce("", String::concat);
	}
	...
	public static class LinEqSolver implements Supplier<int[]>{
		 ...
		 * This is a complete method, you need/should not change it. 
		 * @return
		 */
		Stream<int[]> stream() {
